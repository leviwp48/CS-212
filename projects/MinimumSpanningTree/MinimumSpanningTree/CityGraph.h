...
public:
	...
	{
		if (_graph.find(node->getKey()) == _graph.end())
	...
	{
		if (_graph.find(sink) != _graph.end()) {
	...
	// Creates the reduced graph to be used to make the mst 
	CityGraph createReducedGraph(CityGraph original, CityGraph reduced, vector<vector<string>> deliveries, string& new_source) {

		unordered_map<string, unordered_map<string, int>> distances;
		// store source to call our new map with 
		int count_2 = 0;

		// Creates the reduced graph and inserts nodes
		for (auto visits : deliveries) {
			for (auto locations : visits) {
				distances[locations] = (original.computeShortestPath(locations));
				if (new_source == "") {
					new_source = locations;
				}
				count_2++;
				reduced.addVertex(locations);
			}
		}

		// creates edges between nodes in reduced graph
		int count_3 = 0;
		for (auto source : distances) {
			for (auto edge : source.second) {
				// to prevent putting itself in
				if (edge.first != source.first) {
					reduced.connectVertex(source.first, edge.first, edge.second);
				}
			}
		}
		return reduced;
